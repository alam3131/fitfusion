...
const MyCalendar = ({
  ...
  useEffect(() => {
    ...
    const filteredExercises = workoutsToCalender.filter((event) =>
      isInCurrentDay(event.start),
    );
  ...
  const handleEventClick = (event) => {
    ...
    if (window.confirm(`Are you sure you want to delete "${event.title}"?`)) {
      const updatedEvents = workoutsToCalender.filter(
        (e) => !areEventsEqual(e, event),
      );
      setWorkoutsToCalender(updatedEvents);

      const newPoints =
        tentativePoints - Number(getPointsForLevel(event.Intensity_Level)); // Example: User loses 5 points
  ...
  const handleAddToWorkoutPlan = (exercise) => {
    ...
    const newPoints =
      tentativePoints + Number(getPointsForLevel(exercise.Intensity_Level)); // Example: User earns 10 points
    setTentativePoints(newPoints);
  };
  ...
  const dayPropGetter = (date) => {
    return {
      style: {
        backgroundColor: moment(date).isSame(selectedDate, "day")
          ? "#c8e6c9"
          : "white", // Highlight the selected date
      },
    };
  };
